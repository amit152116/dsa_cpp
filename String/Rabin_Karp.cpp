...
for each string (by using two different p, and/or different m, and compare these pairs instead. If m  is about  
10^9 for each of the two hash functions than this is more or less equivalent as having one hash function with  
m ≈ 10^18 . When comparing 10^6  strings with each other, the probability that at least one collision happens 
